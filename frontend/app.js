window.addEventListener("DOMContentLoaded", async () => {
 ...
 async funcAon loadContractData() {
 if (!contract) return;
...
Number(ethers.formatEther(goal)) * 100);
 ...
 if (!ethAmount || Number(ethAmount) <= 0) return alert("Введите корректное
количество ETH");
...
loadContractData(); }
 ...
 withdrawBtn.onclick = async () => { if (!contract) return alert("Сначала подключите
MetaMask!"); try { await (await contract.withdraw()).wait(); loadContractData(); } catch (err)
{ console.error(err); alert("Ошибка вывода: " + err.message); } };
 refundBtn.onclick = async () => { if (!contract) return alert("Сначала подключите
MetaMask!"); try { await (await contract.refund()).wait(); loadContractData(); } catch (err) {
